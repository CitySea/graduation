			...
			<header class="header">
				<section class="left">
					<section class="header-img left">
						<img :src="base.show_lic">
					</section>
					<section class="header-detail left">
						<h1>{{ base.name }}（{{ base.cnname }}）</h1>
						<p>作者：{{ base.author }}</p>
						<p>连载时间：{{ base.start_date }}</p>
					</section>
          <div class="stars-box">
...
<script>
    ...
    created: function(){
      ...
      vw.base   = vw.dataInfo.base[0];
      vw.bookId = vw.base.id;
      //查看是否收藏该番剧
      vw.$http.post('/api/manage/bookManage/ShowStars', {id: vw.bookId}, {}).then(function(data){
    ...
    methods: {
      ...
      getScore: function(idx){
        ...
        vw.score = idx + 1;
        //评分
        vw.$http.post('/api/manage/bookManage/saveScore', { id: vw.bookId, score: vw.score, name: vw.base.name }, {}).then(function (data){
          if(data.data.code == '11000'){
      ...
      starsTatus: function(){
        ...
        if(vw.isShowStars){
          //取消收藏
          vw.isShowStarsPopup = false;
          vw.$http.post('/api/manage/bookManage/delScore', {id: vw.bookId}, {}).then(function (data){
            if(data.data.code == '11000'){
          ...
          //收藏
          vw.$http.post('/api/manage/bookManage/isLogin', {}, {}).then(function(data){
      ...
      getScoreDetail: function(){
        ...
        vw.scoreArr = [];
        vw.$http.post('/api/manage/bookManage/showScoreDetail', {id: vw.bookId}, {}).then(function(data){
