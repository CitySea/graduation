<template>
	<section class="animation-abstarct">
		<section class="animation-characters">
          ...
          <li class="dynamic-list" v-for="item in journal">
            ...
            <section class="left detail-text">
            	<p class="m1 tag-name"><router-link :to="'/journal/detail?bookId=' + item.id">{{ item.title }}</router-link></p>
            	<br>
      ...
      <section class="edit-fixed"  v-if="isLogin">
        <router-link :to="'/journal/add?bookId=' + bookId">
...
<script>
    ...
    created: function(){
      ...
      vw.bookId = window.location.href.substring(index+3);
